 ...
 * Tag per application and a Session per SDK session id. (Tags are decoupled
 * from tunnels, so no tunnel is needed here.)
 */

const SDK_TAG_SLUG = 'sdk-default';
...
async function ingestSessionPayload(applicationId, payload) {
    ...
    const [tagId, sessionId] = await Promise.all([
        ...
        ensureSession(applicationId, payload.sessionId ?? 'anonymous', visitorId),
    ]);

    // Stamped on every event so the retention aggregation can group by a stable
    // visitor without a join back to the Session.
    const metadata = visitorId ? { visitorId } : {};
    ...
    if (Array.isArray(payload.tabchange?.items) && payload.tabchange.items.length > 0) {
        await Event.create({
            type: 'tabchange',
            payload: { url, items: payload.tabchange.items },
            applicationId,
            sessionId,
            tagId,
            metadata,
        });
        created.push('tabchange');
    }

    if (Array.isArray(payload.submits?.items) && payload.submits.items.length > 0) {
        await Event.create({
            type: 'form_submit',
            payload: { url, items: payload.submits.items },
            applicationId,
            sessionId,
            tagId,
            metadata,
        });
        created.push('form_submit');
    }
