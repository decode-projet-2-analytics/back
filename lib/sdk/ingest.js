 ...
 *   { sessionId, url, flushedAt, page:{viewport,docSize,...}, mousemove?, scroll?, tabchange?, snapshot? }
 *
...
const SDK_TUNNEL_NAME = 'SDK default tunnel';
const SDK_TAG_COMMENT = 'SDK default tag';
const SDK_TAG_SLUG = 'sdk_default';
...
async function ensureDefaultTag(applicationId) {
    ...
    let tunnel = await Tunnel.findOne({
        where: { applicationId, name: SDK_TUNNEL_NAME },
    });
    if (!tunnel) {
        tunnel = await Tunnel.create({ applicationId, name: SDK_TUNNEL_NAME });
    }
...
async function ingestSessionPayload(applicationId, payload) {
    ...
    if (Array.isArray(payload.tabchange?.items) && payload.tabchange.items.length > 0) {
        await Event.create({
            type: 'tabchange',
            payload: { url, items: payload.tabchange.items, docSize, viewport },
            applicationId,
            sessionId,
            tagId,
        });
